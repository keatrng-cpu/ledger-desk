...
export function RiskPanel({
  ...
  desk: DeskPayload;
  /**
   * Parent's already-polled RiskState (index.tsx drives HaltBanner + scanner
   * gating off the same value) so this panel can never show a halt state the
   * rest of the page disagrees with. Omit to self-poll when standalone.
   */
  ...
  useEffect(() => {
    if (!selfPoll) return; // parent owns the poll — never race it
    void load();
  ...
  // Parent value wins when provided, so the HALTED badge here can never
  // disagree with the halt banner / scanner gate elsewhere on the page.
