...
const MODULE_ORDER: Omit<LearnModule, "step">[] = [
  ...
  },
  {
    ...
    desk: `T1 nearest IRL, T2 ERL. Scale ${pct(APLUS_RULES.scaleOut.tp1Fraction)} at T1 (the draw), stop → BE, runner to T2 — banking at +1R instead measured −0.42R/t.`,
    figures: ["liquidity"],
  ...
  },
  {
    ...
    oneLine: "09:30–09:45 ET: name the raid. The window opens only when the raid RESOLVES — then trade the fade.",
    mechanism:
      "The open often exists to trip pre-session stops. Judas is a MODEL, not a hazard: price raids one side, closes back inside the pool it took, and a LATER bar displaces the other way. Outside the killzones the TAPE opens the window, not the clock: delivery ≥2.0×ATR AND volume ≥1.5× its median.",
    rule: "Trade the leg that follows a RESOLVED Judas, on the side the failed raid points to — never the first impulse.",
    trigger:
      "No entries 09:30–09:45 ET until the raid resolves on a 1m/2m/3m rung, then only the fade side at Q ≥ 0.75, paper until it has live evidence. Event cards outside the killzones measured −0.78R/card over four years (thin, both halves negative): the event opens the window, it does not make the trade good. After 10:00 ET, A+ only unless already in.",
    error: "Taking 09:32 as the day's direction — or treating a big candle at 13:00 as permission. More often the trap.",
    desk: `judas-window.ts readJudas() · session-event.ts readSession() · News ±15m. Shock ${SHOCK_RANGE_MULT}× trailing range locks ${SHOCK_LOCK_MS / 60000}m from tape alone.`,
    figures: ["judas"],
  ...
  },
  {
    ...
    mechanism:
      "R = entry→stop. Size = risk $ / R $. The stop has a MEASURED band: 0.5–1.5×ATR. Inside 0.5×ATR it sits in the noise it must survive (−0.48R/card over four years); beyond 1.5×ATR it is a landmark, not a stop (−0.12R/card). Both lose in both halves of the tape and the ticket refuses to size them. Inside the band is roughly breakeven — the band's value is the losses it refuses.",
    rule: "Don't size a stop you haven't placed. Don't widen after entry.",
    trigger: `Min ${APLUS_RULES.minRr.toFixed(1)}:1 · TP clamp ${APLUS_RULES.tpMaxR.toFixed(0)}R is DISPLAY-only (as a T1 cap it measured −0.12R/t) · stop 0.5–1.5×ATR · A+ ${pct(APLUS_RULES.riskByGrade["A+"])} · A ${pct(APLUS_RULES.riskByGrade.A)} · A− ${pct(APLUS_RULES.riskByGrade["A-"])} · B+ ${pct(APLUS_RULES.riskByGrade["B+"])}. A+ probe ${pct(APLUS_PROBE_RISK)} until n≥${APLUS_FULL_SIZE_MIN_N} A+ WR≥${pct(APLUS_FULL_SIZE_MIN_WR)}, then ${pct(APLUS_FULL_RISK)}.`,
    error: "Sizing off a mental stop, or widening so the loss 'isn't real'.",
    desk: `T1 (the draw) banks ${pct(APLUS_RULES.scaleOut.tp1Fraction)}, stop → BE, runner to T2. One full A loss (2%) is the whole daily halt. Daily halt ${pct(APLUS_RULES.dailyLossLimitPct)} · weekly ${pct(APLUS_RULES.weeklyLossLimitPct)}.`,
    figures: ["risk"],
  ...
  },
  {
    ...
    trigger: `Q is a FIT score, not odds. The card used to flash at ≥${HIGH_CONFLUENCE_THRESHOLD.toFixed(2)}; over four years Q 0.85+ went the card's way 46% of the time vs 54% at 0.65–0.70. A high Q is a reason to check the stop band and the sequence harder, never to take. Sweep-polarity can still refuse.`,
